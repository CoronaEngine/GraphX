...
13. While `IMPLEMENTING`, answer status requests by validating `.polaris/tasks/<TASK>/runtime/progress.json` and formatting its fields directly in the conversation. Emit `IMPLEMENTATION_PROGRESS` with the latest phase, current action, completed steps, remaining steps, checks, blocker, timestamp, and task link. Do not persist a duplicate Markdown status file, invent percentages, or infer progress from elapsed time. A status query must not cancel or duplicate the worker.
