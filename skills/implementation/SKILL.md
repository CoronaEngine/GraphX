...
2. Confirm state is `IMPLEMENTING`, the handoff hash matches `state.json`, and `artifact_attempt`, revision, base commit, output path, and progress paths are current. Invoke `{{skill:code-intelligence}}` before editing for optional context and impact observations derived only from the handoff package. Missing or failing provider operations immediately fall back to the original source-reading path. Refresh during Implementation only when a later declared step needs relationships from newly changed code.
