...
4. Assign a reviewer session ID distinct from the implementer for R1/R2. Attest truthfully to isolation and chat-history inheritance; do not fabricate independence. Invoke `{{skill:code-intelligence}}` for optional independent `review_context` and impact observations derived only from the registered subject. Do not reuse Implementer query conclusions. Missing or failing provider operations immediately fall back to the original frozen-package and source review.
